...
class NaveLista(ListView):
    ...
    def get_queryset(self):
        ...
        qs = (Nave.objects.select_related("armador").annotate(cer_vigentes=Count("certificados",filter=Q(certificados__fecha_vencimiento__gte=self.request.today))))
        if q:
...
class NaveDetalle(DetailView):
    model = Nave
    def  get_queryset(self):
        return (Nave.objects.select_related("armador").prefetch_related("certificados"))
